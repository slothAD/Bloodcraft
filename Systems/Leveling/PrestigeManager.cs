...
internal static class PrestigeManager
{
    ...
    public static void DisplayPrestigeInfo(ChatCommandContext ctx, ulong steamId, PrestigeType parsedPrestigeType, int prestigeLevel, int maxPrestigeLevel)
    {
        ...
        if (parsedPrestigeType == PrestigeType.Experience)
        {
            ...
            ctx.Reply($"<color=#90EE90>{parsedPrestigeType}</color> 進階資訊：");
            ctx.Reply($"當前進階等級：<color=yellow>{prestigeLevel}</color>/{maxPrestigeLevel}");
            ctx.Reply($"專精與傳承的成長速度提升：<color=green>{gainPercentage}</color>");
            ctx.Reply($"經驗獲取速度降低：<color=yellow>{reductionPercentage}</color>");

            if (prestigeData.TryGetValue(PrestigeType.Exo, out var exoData) && exoData > 0)
            {
                ctx.Reply($"經驗獲取速率的減益在進行 Exo 進階時不再適用。");
            }
        }
        else
        {
            ...
            LocalizationService.HandleReply(ctx, $"<color=#90EE90>{parsedPrestigeType}</color> 進階資訊：");
            LocalizationService.HandleReply(ctx, $"當前進階等級：<color=yellow>{prestigeLevel}</color>/{maxPrestigeLevel}");
            LocalizationService.HandleReply(ctx, $"<color=#90EE90>{parsedPrestigeType}</color> 進階等級帶來的成長速率下降：<color=yellow>-{percentageReductionString}</color>");
            LocalizationService.HandleReply(ctx, $"屬性加成提升：<color=green>{statGainString}</color>");
            LocalizationService.HandleReply(ctx, $"總成長速率變化（含等級進階加成）：<color=yellow>{totalEffectString}</color>");
        }
    }
    ...
    static void HandleExperiencePrestige(ChatCommandContext ctx, int prestigeLevel, KeyValuePair<int, float> xpData)
    {
        ...
        */
        ...
        LocalizationService.HandleReply(ctx, $"你已在 <color=#90EE90>經驗</color> 進階達到 [<color=white>{prestigeLevel}</color>]！所有專精與傳承成長速率提升 <color=green>{gainPercentage}</color>，擊殺單位所得經驗降低 <color=red>{reductionPercentage}</color>。");
    }
    static void HandleOtherPrestige(ChatCommandContext ctx, ulong steamId, PrestigeType parsedPrestigeType, int prestigeLevel)
    {
        ...
        */
        ...
        LocalizationService.HandleReply(ctx, $"<color=#90EE90>{parsedPrestigeType}</color>[<color=white>{prestigeLevel}</color>] 進階成功！成長速率降低 <color=red>{percentageReductionString}</color>，屬性加成提升 <color=green>{statGainString}</color>。總體成長速率變化（含等級進階加成）：<color=yellow>{totalEffectString}</color>。");
    }

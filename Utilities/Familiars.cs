...
internal static class Familiars
{
    ...
    public static void ParseAddedFamiliar(ChatCommandContext ctx, ulong steamId, string unit, string activeBox = "")
    {
        ...
        if (int.TryParse(unit, out int prefabHash) && PrefabCollectionSystem._PrefabGuidToEntityMap.TryGetValue(new(prefabHash), out Entity prefabEntity))
        {
            ...
            {
                LocalizationService.HandleReply(ctx, "無效的單位預設值（雖有匹配，但未以 CHAR/char 開頭）。");
                return;
            }
            ...
            LocalizationService.HandleReply(ctx, $"<color=green>{new PrefabGUID(prefabHash).GetLocalizedName()}</color> 已新增至 <color=white>{activeBox}</color>。");
        }
        else if (unit.ToLower().StartsWith("char")) // search for full and/or partial name match
        {
            ...
            if (!match.IsEmpty() && PrefabCollectionSystem._PrefabGuidToEntityMap.TryGetValue(match, out prefabEntity))
            {
                if (!prefabEntity.Read<PrefabGUID>().GetPrefabName().StartsWith("CHAR"))
                {
                    LocalizationService.HandleReply(ctx, "無效的單位名稱（雖有匹配，但未以 CHAR/char 開頭）。");
                    return;
                }
                ...
                LocalizationService.HandleReply(ctx, $"<color=green>{match.GetLocalizedName()}</color> (<color=yellow>{match.GuidHash}</color>) 已新增至 <color=white>{activeBox}</color>。");
            }
            else
            {
                LocalizationService.HandleReply(ctx, "無效的單位名稱（找不到完整或部分匹配）。");
            }
        }
        else
        {
            LocalizationService.HandleReply(ctx, "無效的預設值（非整數）或名稱（未以 CHAR/char 開頭）。");
        }
    }
    ...
    public static void HandleFamiliarPrestige(ChatCommandContext ctx, int clampedCost) // need to replace first block in command with this method but laterrrr
    {
        ...
        {
            LocalizationService.HandleReply(ctx, $"你的使魔已達進階次數上限！（<color=white>{ConfigService.MaxFamiliarPrestiges}</color>）");
            return;
        }
        ...
        */
        ...
        if (ServerGameManager.TryRemoveInventoryItem(playerCharacter, _itemSchematic, clampedCost))
        {
            ...
            LocalizationService.HandleReply(ctx, $"你的使魔已完成第 <color=#90EE90>{prestigeLevel}</color> 次進階；其積累的智慧讓它保留了等級！");
            ...
            {
                LocalizationService.HandleReply(ctx, $"你的使魔已完成第 <color=#90EE90>{prestigeLevel}</color> 次進階；其積累的智慧讓它保留了等級！（+<color=#00FFFF>{FamiliarPrestigeStats[value]}</color>）");
            }
            */
        }
        else
        {
            LocalizationService.HandleReply(ctx, "從背包中移除設計圖失敗！");
        }
    }

...
internal static class Quests
{
    ...
    public static void QuestTrackReply(ChatCommandContext ctx, Dictionary<QuestType, (QuestObjective Objective, int Progress, DateTime LastReset)> questData, QuestType questType)
    {
        ...
        if (questData.TryGetValue(questType, out var questObjective) && questObjective.Objective.Goal.Equals(TargetType.Kill) && !questObjective.Objective.Complete)
        {
            ...
            {
                LocalizationService.HandleReply(ctx, $"所有目標已被擊殺，請稍待其重生！如與預期不符，可考慮重擲你的 {QuestTypeColor[questType]} 任務。");
            }
            else if (entities.Count > 0)
            {
                ...
                if (!closest.Exists())
                {
                    LocalizationService.HandleReply(ctx, "所有目標已被擊殺，請稍待其重生！");
                    return;
                }
                else if (closest.IsVBloodOrGateBoss())
                {
                    LocalizationService.HandleReply(ctx, "請使用 VBlood 菜單來追蹤首領！");
                    return;
                }
                ...
                LocalizationService.HandleReply(ctx, $"最近的 <color=white>{questObjective.Objective.Target.GetLocalizedName()}</color> 曾在 <color=#00FFFF>{(int)distance}</color>f 外的 {cardinalDirection} 方向出現於 <color=#F88380>{(int)seconds}</color> 秒前。");
            }
            else if (entities.Count == 0)
            {
                LocalizationService.HandleReply(ctx, "所有目標已被擊殺，請稍待其重生！");
        ...
        {
            LocalizationService.HandleReply(ctx, "追蹤功能僅適用於尚未完成的擊殺任務。");
        }
    }
    */
    ...
    public static void QuestObjectiveReply(ChatCommandContext ctx, Dictionary<QuestType, (QuestObjective Objective, int Progress, DateTime LastReset)> questData, QuestType questType)
    {
        if (questData.TryGetValue(questType, out var questObjective) && !questObjective.Objective.Complete)
        {
            ...
            LocalizationService.HandleReply(ctx, $"{questType} 任務重置倒數：<color=yellow>{timeLeft}</color> | 目標：<color=white>{questObjective.Objective.Target.GetPrefabName()}</color>");
        }
        else if (questObjective.Objective.Complete)
        {
            string timeLeft = GetTimeUntilReset(questObjective, questType);
            LocalizationService.HandleReply(ctx, $"你已完成 {QuestTypeColor[questType]}！任務將於 <color=yellow>{timeLeft}</color> 後重置。");
        }
        else
        {
            LocalizationService.HandleReply(ctx, $"你尚未擁有 {QuestTypeColor[questType]} 任務。");
        }
    }

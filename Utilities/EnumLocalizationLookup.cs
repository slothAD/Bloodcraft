...
public static class EnumLocalizationLookup
{
    public static string GetWeaponTypeZh(WeaponType type)
        => EnumLocalization.WeaponTypeToZh.TryGetValue(type.ToString(), out var name) ? name : type.ToString();

    public static string GetBloodTypeZh(BloodType type)
        => EnumLocalization.BloodTypeToZh.TryGetValue(type.ToString(), out var name) ? name : type.ToString();

    public static string GetProfessionZh(ProfessionType type)
        => EnumLocalization.ProfessionTypeToZh.TryGetValue(type.ToString(), out var name) ? name : type.ToString();

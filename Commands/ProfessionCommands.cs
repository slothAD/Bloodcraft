...
internal static class ProfessionCommands
{
    ...
    public static void GetProfessionCommand(ChatCommandContext ctx, string profession)
    {
        ...
        if (data.Key > 0)
        {
            ...
            LocalizationService.HandleReply(ctx, $"你目前在 {professionHandler.GetProfessionName()} 的等級為 [<color=white>{data.Key}</color>]，擁有 <color=yellow>{progress}</color> 點<color=#FFC0CB>熟練度</color>（<color=white>{ProfessionSystem.GetLevelProgress(steamId, professionHandler)}%</color>）");
        }
        else
        {
            LocalizationService.HandleReply(ctx, $"尚未在 {professionHandler.GetProfessionName()} 開始進度！");
        }
    }
    ...
    public static void SetProfessionCommand(ChatCommandContext ctx, string name, string profession, int level)
    {
        ...
        LocalizationService.HandleReply(ctx, $"{professionHandler.GetProfessionName()} 等級設為 [<color=white>{level}</color>]，目標玩家：<color=green>{playerInfo.User.CharacterName.Value}</color>");
    }
